...
TARGET_OBJECT = "[工八_PEREADONLY].[LK2MES-DB-REAL].[dbo].[V_PE_PRD_TestResult_BER_Symbol_Error_Test]"
...
TC_BER_ITEM_SET = {item.lower() for item in TC_BER_ITEMS}
...
def is_tc_ber_channel(value: object) -> bool:
    ...
    return text in TC_BER_ITEM_SET
...
def main():
    ...
    try:
        ...
        with pyodbc.connect(conn_str_with_db(DATABASE), timeout=30) as conn:
            ...
            categories = [("TC_BER", "TC BER"), ("其他", "其他")]
            ...
            for category_key, sheet_name in categories:
                sheet_df = df[df["_category"] == category_key].drop(columns=["_category"])
                if sheet_df.empty:
                    sheet_df = df.head(0).drop(columns=["_category"])
                write_dataframe_to_sheet(workbook, sheet_name, sheet_df)
